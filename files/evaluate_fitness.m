...
if strcmp(side, 'top')
    [min_y, idx] = min(py);
    point = [px(idx); min_y];
    cost = mean(vecnorm([x;y]-point)) + 2*norm(extremum(2) - min_y);
    if min_y < extremum(2)
        cost = cost + 1e5*(extremum(2) - min_y);
    end
    ...
    [max_y, idx] = max(py);
    point = [px(idx); max_y];
    cost = mean(vecnorm([x;y]-point)) + 2*norm(extremum(2) - max_y);
    if max_y > extremum(2)
        cost = cost + 1e5*(max_y - extremum(2));
    end
